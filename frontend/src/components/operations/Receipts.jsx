// src/components/operations/Receipts.jsx
import React, { useState } from 'react';
import OperationForm from './OperationForm';

const Receipts = ({ products = [], onReceiptCreated }) => {
  const [formData, setFormData] = useState({
    contact_name: '', // Changed from supplier_name to contact_name
    product_id: '',
    quantity: '',
    notes: '' // New field
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('DEMO: Would send to backend:', {
        supplier_name: formData.contact_name, // Map to supplier_name for backend
      ...
      const result = {
        id: Date.now(),
        type: 'RECEIPT',
        supplier_name: formData.contact_name, // Keep supplier_name in response
        product_id: parseInt(formData.product_id),
        quantity: parseInt(formData.quantity),
        notes: formData.notes,
      ...
      setMessage('✅ Stock received successfully! (Demo Mode)');
  ...
  return (
    <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
      ...
      <OperationForm
        ...
        submitButtonText="✅ Receive Stock"
      />

      {/* ✅ ADD DOWNLOAD BUTTON HERE */}
      <div className="mt-4 pt-4 border-t border-slate-200">
        <button
          onClick={() => console.log('Download receipt functionality')}
